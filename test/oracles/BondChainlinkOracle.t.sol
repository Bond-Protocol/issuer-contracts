...
contract BondChainlinkOracleTest is Test {
    ...
    function test_setPair_noZeroAddress() public {
        ...
        bytes memory err = abi.encodeWithSignature("BondOracle_InvalidParams()");
        vm.expectRevert(err);
        oracle.setPair(MockERC20(address(0)), tokenOne, true, abi.encode(1, 1, 1, 1, 1, 1));

        // Try to set pair with second token zero address, should revert
        vm.expectRevert(err);
        oracle.setPair(tokenOne, MockERC20(address(0)), true, abi.encode(1, 1, 1, 1, 1, 1));
    ...
    function test_setPair_remove() public {
        ...
        bytes memory err = abi.encodeWithSignature("BondOracle_BadFeed(address)", address(0));
        vm.expectRevert(err);
        oracle.currentPrice(0);
    }
}
