...
contract BondUniV3Oracle is BondBaseOracle {
        ...
        returns (uint256)
    {
        ...
        if (address(params.denominatorPool) == address(0)) {
            ...
            uint256 numerator = _validateAndGetPrice(
                address(payoutToken_),
                params.numeratorPool,
                params.observationWindowSeconds,
                params.decimals
    ...
    function _validateAndGetPrice(
        ...
        // Get tick from pool
        (int24 timeWeightedTick, ) = OracleLibrary.consult(
            address(pool_),
        ...
        uint256 tokenPrice = OracleLibrary.getQuoteAtTick(
            int24(timeWeightedTick),
            uint128(10**ERC20(token_).decimals()),
            token_,
            quoteToken
        );
    ...
    function _setPair(
        ...
        if (supported_) {
            ...
            {
                uint32 oldestSecondsAgo = OracleLibrary.getOldestObservationSecondsAgo(
                    address(params.numeratorPool)
                );
                if (params.observationWindowSeconds > oldestSecondsAgo)
                    revert BondOracle_InvalidParams();
            }
            ...
            if (address(params.denominatorPool) == address(0)) {
                ...
                // Check that the numerator pool is a contract
                if (address(params.denominatorPool).code.length == 0)
                    revert BondOracle_InvalidParams();

                // Check that the observationWindowSeconds is less than or equal to the max on the numerator pool
                {
                    uint32 oldestSecondsAgo = OracleLibrary.getOldestObservationSecondsAgo(
                        address(params.denominatorPool)
                    );
                    if (params.observationWindowSeconds > oldestSecondsAgo)
                        revert BondOracle_InvalidParams();
                }
                ...
                address denomInterToken = denomToken0 == address(quoteToken_)
                    ? denomToken1
                    : denomToken0;
